<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
</head>
<body>
    <table border="1">
        <tr>
            <th>Faktur Komersial/ <i>Commercial Invoice</i></th>
            <th colspan="2"><img src="tokpeeddd.PNG" width="200px"></th>
            <th colspan="1">
                Tanggal Pembelian : <br>
                <input type="date">
            </th>
        </tr>
        <tr>
            <td height="50px">Nama Penjual : <b>Michael Johnson</b></td>
            <td rowspan="2">Alamat Penjual <br>
                Jl. Mawar Melati,Perum Bunga Indah Blok A7 No 1 <br>
                Kec. Sentosa, Kota Samarinda, Kalimantan Timur, 12345 <br>
                Kalimantan Timur</td>
            <td rowspan="2" colspan="2">Alamat Pembeli <br>
                Jl. Semua Indah, Perum Bunga Indah Blok Z7 No 1 <br>
                Kec. Miramar, Kota Samarinda, Kalimantan Timur, 12345 <br>
                Kalimantan Timur</td>
            </td> 
        </tr>
        <tr>
            <td height="50px">Nama Pembeli : <b>Miya Yoshitaka</b></td>
        </tr>
        <tr>
            <td>info Produk</td>
            <td>Jumlah</td>
            <td>Harga Satuan</td>
            <td>Total Harga</td>
        </tr>
        <tr>
            <td height="50px">Nike Trinity Force Angel</td>
            <td>
                <input type="number"> pcs
            </td>
            <td>Rp 799.000</td>
            <td>Rp 799.000</td>
        </tr>
        <tr>
            <td rowspan="3" colspan="2"></td>
            <td>Total Belanja</td>
            <td>Rp 799.000</td>
        </tr>
        <tr>
            <td>Total Ongkos Kirim</td>\
            <td>Rp 99.000</td>
        </tr>
        <tr>
            <td>Total Tagihan</td>
            <td><b>Rp 898.000</b></td>
        </tr>
        <tr>
            <td>Kurir :<br>
            GNZ Ult - Ultimate</td>
            <td colspan="3">Metode Pembayaran : <br>
                <input type="radio" name="gender"> Bank BCA
                <input type="radio" name="gender"> Bank Jago
                <input type="radio" name="gender"> Bank Mandiri
                <input type="radio" name="gender"> Bank Mega
                <input type="radio" name="gender"> Bank Kaltimtara
            </td>
        </tr>
        <tr>
            <td colspan="4" rowspan="1">
                <ul>
                    <li> Nama : Muhammad Al Hafni </li>
                    <li> NIM : 2209106119 </li>
                    <li> Praktikum Web : <a href="https://classroom.google.com/c/NjIxNzUxOTQ4NjI1/a/NjIzNjU2MTY5MjI0/details">Posttest 1</a></li>

                </ul>
            </td>
        </tr>
    </table>
</body>
</html>
