...
<head>
    ...
    <style>
        ...
        .field {
            border-bottom: 1px solid rgba(255, 255, 255, 0.2); /* Garis pemisah putih dengan opacity 0.2 */
            padding-bottom: 2px;
            margin-bottom: 20px;
            display: flex;
            justify-content: space-between;
...
<body>
    ...
    <div class="container">
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            // Mocked up for the example. You might want to use database or other mechanisms to generate and manage these values.
            $tanggalOrder = date("Y-m-d H:i:s"); // get current date and time
            $nomerTransaksi = "TX" . time(); // get current timestamp as a unique transaction ID

            // Other details
            $statusPembayaran = "Cash on Delivery"; // Cash on Delivery
            $statusTransaksi = "Pending";
            $totalPembayaran = 20000 * intval($jumlah); // assuming each chocolate is 20,000

            // Display order confirmation with new styles
